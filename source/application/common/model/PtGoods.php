...
class PtGoods extends BaseModel
{
    ...
    {
        return $this->belongsTo('PtCategory');
    }
    ...
    {
        return $this->hasMany('PtGoodsSpec');
    }
    ...
    {
        return $this->belongsToMany('SpecValue', 'PtGoodsSpecRel');
    }
    ...
    {
        return $this->hasMany('PtGoodsImage')->order(['id' => 'asc']);
    }
    ...
    public function getList($status = null, $category_id = 0, $search = '', $sortType = 'all', $sortPrice = false)
    {
        // 筛选条件
        $filter = [];
        $category_id > 0 && $filter['pt_category_id'] = $category_id;
        ...
        $sort = [];
        if ($sortType === 'all') {
            $sort = ['goods_sort', 'pt_goods_id' => 'desc'];
        ...
        // 多规格商品 最高价与最低价
        $GoodsSpec = new PtGoodsSpec;
        $minPriceSql = $GoodsSpec->field(['MIN(goods_price)'])
            ->where('pt_goods_id', 'EXP', "= `$tableName`.`pt_goods_id`")->buildSql();
    ...
    public function getNewList()
    {
        return $this->with(['spec', 'category', 'image.file'])
            ->where('is_delete', '=', 0)
            ->where('goods_status', '=', 10)
            ->order(['pt_goods_id' => 'desc', 'goods_sort' => 'asc'])
            ->select();
    }
